...
\hypersetup{
    ...
    pdfsubject={Quantum Computing and Buddhist Philosophy},
...
% Complete solution: fancyhdr + page counter
...
% Environment for epistemological paradoxes
\newenvironment{epistemic_warning}[1]{
    ...
    \hrule
    \vspace{0.2cm}
    \textbf{EPISTEMOLOGICAL WARNING: #1}
    \vspace{0.1cm}
    \hrule
    \vspace{0.2cm}
}{
...
\title{
    \textbf{Quantum Simulation of the Bardo Thodol Consciousness States:} \\
    \large A Computational Approach from Qutrit Theory and Karmic Dynamics \\
...
\begin{titlepage}
    ...
    \vspace{0.5cm}
    {\Large Modeling Post-Mortem Consciousness States \\ through Qutrit Systems and Karmic Operators}

    \vspace{0.3cm}
    {\normalsize \textit{With Explicit Epistemological Metamodeling}}
    ...
    \begin{abstract}
        \noindent This article presents an innovative theoretical and computational framework for the quantum simulation of consciousness states described in the \emph{Bardo Thodol} (Tibetan Book of the Dead). We propose a model based on qutrit systems (three-level quantum states) where post-mortem states are represented as quantum superpositions, and karmic transitions as temporal evolution operators dependent on attention parameters and karmic accumulations.

        \vspace{0.3cm}
        \noindent Following the Madhyamaka method of the Two Truths, this work makes explicit the unresolvable paradoxes inherent to mathematical modeling of contemplative phenomena, distinguishing between conventional truth (\emph{saṃvṛti-satya}), ultimate truth (\emph{paramārtha-satya}), and pedagogical use (\emph{upāya}).

        \vspace{0.3cm}
        \noindent \textbf{Keywords:} Bardo Thodol, Quantum Computing, Qutrits, Consciousness States, Simulation, Sunyata, Karma, Quantum Decoherence, Epistemology of Modeling
    \end{abstract}

    \vfill
    {\small Project started in January 2025 · Updated in November 2025}
\end{titlepage}
...
The \emph{Bardo Thodol}, traditionally interpreted as a ritual guide for post-mortem transition in the Tibetan tradition, is reformulated in this work as an \textbf{ancestral algorithm} that encodes the fundamental dynamics of consciousness states. This reinterpretation is situated at the intersection of:
...
\begin{itemize}
    ...
    \item \textbf{Critical Epistemology}: Reflective analysis of formal modeling limits
\end{itemize}
...
\end{equation}
with $\ket{0}$ representing the state of manifest reality (samsara), $\ket{1}$ karmic potential states, and $\ket{2}$ fundamental emptiness (sunyata), where $|\alpha|^2 + |\beta|^2 + |\gamma|^2 = 1$.
\end{definition}

\begin{epistemic_warning}{Truth Level}
This mathematical representation belongs to the conventional level (\emph{saṃvṛti-satya}). At the ultimate level (\emph{paramārtha-satya}), śūnyatā is not a measurable vector state but the empty nature of all phenomena, including the very concept of emptiness. The model is \textbf{upāya} (skillful means) pedagogical, not ontological description.
\end{epistemic_warning}

\subsection{Scientific Justification}

The need for a quantum approach arises from the fundamental limitations of classical computational models:

\begin{itemize}
    \item \textbf{Dualism Problem}: Binary systems cannot capture the non-dual nature of emptiness
    \item \textbf{Turing Limitations}: The classical machine cannot represent coherent superpositions
    \item \textbf{Probabilistic Nature}: The karmic process is intrinsically probabilistic, not deterministic
\end{itemize}

\subsection{Epistemological Transparency: Modeling Limits}

This work adopts the method of \textbf{Two Truths} (Madhyamaka) applied to computational simulation:

\begin{definition}[Truth Levels in the Model]
\begin{enumerate}
    \item \textbf{Conventional Level} (\emph{saṃvṛti-satya}): Quantum mathematics is formally valid in its domain
    \item \textbf{Ultimate Level} (\emph{paramārtha-satya}): The formalism does not capture śūnyatā as ultimate reality
    \item \textbf{Pedagogical Use} (\emph{upāya}): The model is heuristic tool for exploration, not identity with the phenomenon
\end{enumerate}
\end{definition}

\subsubsection{Documented Unresolvable Paradoxes}

\begin{paradox}[Karmic Quantification]
\label{paradox:karma}
Assigning numerical values to karma (e.g., $k_{\text{clarity}}=0.8$, $k_{\text{attachment}}=0.3$) \textbf{reifies} what the Abhidharma describes as impermanent flow (\emph{anitya}) without fixed substance (\emph{anātman}).
...
\textbf{Pedagogical value:} The parameters allow exploring how different habitual tendencies affect transitions, without claiming that karma \emph{is} these numbers. Heuristic function, not descriptive.
\end{paradox}

\begin{paradox}[Reification of Emptiness]
\label{paradox:sunyata}
Representing śūnyatā as vector $\ket{2} = [0,0,1]^T$ in Hilbert space contradicts its nature of \emph{niḥsvabhāva} (absence of inherent being).

\textbf{Irreducible gap:} Converting emptiness into separate mathematical state is exactly the type of reification (\emph{saṃjñā}) that Prajñāpāramitā warns to avoid. It is an unresolvable performative contradiction.

\textbf{Pedagogical value:} Demonstrates the need for non-binary frameworks that surpass classical logic. $\ket{2}$ does not \emph{is} emptiness, it \emph{points} towards it as a finger pointing to the moon.
\end{paradox}

\begin{paradox}[Model Temporality]
\label{paradox:time}
The temporal evolution $\unitary(t) = e^{-i\hamiltonian t}$ requires time as continuous parameter, while in deep meditative states (\emph{samādhi}), temporal experience collapses.

\textbf{Irreducible gap:} The mathematical formalism cannot model atemporal experience without structural self-contradiction. \emph{Kāla} (time) is mental construction, not absolute.

\textbf{Pedagogical value:} Shows dynamics of transitions as sequential process useful for conceptual understanding. The user must remember that mathematical time is artifact of the model.
\end{paradox}

\begin{table}[h]
\centering
\caption{Metamodeling: Conventional Truth vs Ultimate Truth}
...
Measurement & Collapse $\ket{\psi} \to \ket{i}$ & \emph{Rigpa} non-dual without observer \\
...
We define our three-dimensional Hilbert space to model the fundamental states:

\begin{equation}
\mathcal{H} = \text{span}\{\ket{0}, \ket{1}, \ket{2}\}
\end{equation}

With the corresponding projection operators:
...
\ket{2} &= \begin{bmatrix} 0 \\ 0 \\ 1 \end{bmatrix} \quad \text{(Points towards Sunyata)}
\end{align*}
\end{definition}

\begin{epistemic_warning}{Interpretation of Basis States}
The three basis vectors are NOT ontologically separate realities. At the ultimate level, all states interpenetrate without fixed boundary. The mathematical separation is pedagogical convention for formal analysis.
\end{epistemic_warning}
...
where $k_{ij}$ represents the karmic couplings between states (subject to Paradox~\ref{paradox:karma}) and $\epsilon_i$ the intrinsic potentials of each state.

\subsection{The Six Bardos as Quantum Transitions}

We model the six Bardo states as sequences of quantum transitions:

\begin{enumerate}
    \item \textbf{Bardo of the Moment of Death (Chikhai Bardo)}: $\ket{2} \otimes \ket{k}$
    \item \textbf{Bardo of Reality (Chonyid Bardo)}: $\sum_k c_k\ket{k}$
    \item \textbf{Bardo of Becoming (Sidpa Bardo)}: $\ket{0} \leftarrow$ Measurement
\end{enumerate}

\subsection{Conceptual Genesis: From ERROR 505 to Quantum Qutrit}

The conceptual turning point emerged from the analysis of anthropomorphic digital classifications applied to post-mortem consciousness states. The identification of "ERROR 505" as "Lack of deity recognition" revealed a fundamental limitation in classical computational models.

\subsubsection{Limitation of the Binary Paradigm}

The interpretation as "error" emerged from a binary framework unable to represent:
\begin{itemize}
    \item States of non-collapsed quantum superposition
...
The resolution required transcending Boolean logic through:
\begin{equation}
\mathcal{H}_{\text{Bardo}} = \alpha\ket{0} + \beta\ket{1} + \gamma\ket{2}
\end{equation}
where $\ket{2}$ points towards fundamental emptiness, not an error state.

This paradigmatic transition allowed reinterpreting "errors" as windows to states of maximum quantum potentiality where karma can be reprogrammed.
...
We implemented the system using Python 3.11 with the following main libraries:
...
class BardoQuantumSystem:
    ...
    def __init__(self, karma_params=None):
        ...
        self.model_limitations = {
            'karma_reification':
                'Numerical parameters reify impermanent flow',
            'temporal_assumption':
...
\subsection{Temporal Evolution Algorithm}

The main algorithm simulates the complete evolution through the Bardo states:
...
def simulate_bardo_transition(self, time_steps=1000,
    ...
    """Simulates transition with documentation of assumptions"""
...
def _generate_epistemic_note(self, state, time):
    """Generates note about model limits at this point"""
    probs = [qt.expect(self.operators[f'P{i}'], state)
             for i in range(3)]
    dominant = np.argmax(probs)

    notes = {
        0: f"t={time:.2f}: High P(|0>) indicates manifestation, "
           f"but form is empty",
...
\subsection{Temporal Evolution of Probabilities}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/state_evolution.pdf}
    \caption{
        Temporal evolution of probabilities and quantum metrics in the Bardo system (Conventional Level).
        (A) Probabilities of fundamental states: Samsara ($\ket{0}$), Karmic Potential ($\ket{1}$) and pointer to Emptiness ($\ket{2}$).
        (B) Evolution of quantum coherence and state purity, showing periods of coherent superposition and decoherence.
...
Quantum coherence is maintained during transitions between Bardos, with characteristic patterns:

\begin{equation}
C(\rho) = \sum_{i\neq j} |\rho_{ij}|
\end{equation}

\begin{epistemic_warning}{Interpretation of Coherence}
Mathematical quantum coherence is ANALOGOUS (not identical) to phenomenological "non-dual interpenetration". The number $C(\rho)$ does not directly measure contemplative clarity (\emph{prajñā}), but points towards it as formal correlate.
\end{epistemic_warning}
...
\subsection{Visualization of Karmic Transitions}

\begin{lstlisting}[caption={Generation of scientific visualizations}, label={lst:visualization}]
def create_comprehensive_visualization(results, times):
    """Creates visualizations with epistemological notes"""
    ...
    ax1.plot(times, probabilities[:, 1],
             label='$|1\\rangle$ Karmic', linewidth=2)
    ax1.plot(times, probabilities[:, 2],
             label='$|2\\rangle$ Emptiness', linewidth=2)
    ax1.set_xlabel('Time')
    ...
    fig.text(0.5, 0.02,
             'Conventional Level: Formally valid metrics',
...
\subsection{Validation of Central Hypothesis}

Our results demonstrate that (at the conventional level):

\begin{enumerate}
    \item The qutrit model can effectively represent the non-duality of emptiness \textit{as logical structure}
    \item Transitions between Bardo states follow coherent quantum dynamics \textit{as formal analogy}
    \item The metaphorical "ERROR 505" corresponds mathematically to non-collapsed superposition states \textit{pointing towards potentiality}
\end{enumerate}

\begin{epistemic_warning}{Validation Scope}
The "validation" is internal to the mathematical model. We do not claim that the Bardo Thodol "is" a quantum algorithm, but that quantum formalism can be used as \emph{upāya} to explore its logical structure. The contemplative post-mortem experience remains outside the model's scope.
\end{epistemic_warning}
...
\begin{tabular}{lcc}
\toprule
\textbf{Characteristic} & \textbf{Classical Model} & \textbf{Quantum Model} \\
\midrule
...
\textbf{Documented paradoxes} & \textbf{Ignored} & \textbf{Explicitated} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item Consciousness states could follow quantum dynamics \textit{in certain structural aspects}
    \item Deep meditation could affect quantum coherence parameters \textit{measurable neurophysiologically}
    \item Computational models of consciousness should consider quantum frameworks \textit{along with explicit limitation statements}
\end{itemize}

\subsection{Recognized Project Limitations}

\begin{enumerate}
    \item \textbf{Phenomenological gap}: The model does not capture direct experience (\emph{pratyakṣa}) of bardo states
    \item \textbf{Parametric reductionism}: Quantified karma contradicts its nature as interdependent process
    \item \textbf{Artificial temporality}: Mathematical time does not reflect atemporality of \emph{samādhi}
    \item \textbf{Observational dualism}: Maintains measurer-measured separation absent in \emph{rigpa}
    \item \textbf{Reification of emptiness}: $\ket{2}$ as vector contradicts \emph{niḥsvabhāva}
\end{enumerate}

These limitations are not "problems to solve" but inherent characteristics of mathematical modeling of contemplative phenomena.
...
\begin{enumerate}
    \item We have demonstrated the feasibility of \textit{structurally} modeling Bardo Thodol consciousness states using quantum systems
    \item The qutrit approach overcomes limitations of binary models \textit{at the level of formal logic}
    \item Emptiness (sunyata) finds natural mathematical representation in quantum superpositions \textit{as analogy, not identity}
    \item Karmic dynamics can be implemented as quantum operators \textit{with explicit recognition of reification}
    \item \textbf{The explication of unresolvable paradoxes} turns the project into reflexive metamodel
\end{enumerate}

\subsection{Methodological Framework: The Three Truths Applied}

\begin{table}[h]
\centering
\caption{Application of the Two Truths method to modeling}
...
\begin{itemize}
    \item \textbf{Experimental Validation}: Integration with advanced meditation data and EEG, \textit{with warning that neural correlates are not the experience}
    \item \textbf{Quantum Hardware}: Implementation in real quantum processors (IBM Q, Rigetti), \textit{as demonstration of computational feasibility}
    \item \textbf{Extended Models}: Generalization to multi-state systems and dimensions, \textit{maintaining epistemological transparency}
    \item \textbf{Clinical Applications}: Potential applications in therapy and altered states of consciousness, \textit{without psychological reductionism}
    \item \textbf{Contemplative-scientific dialogue}: Validation with advanced practitioners about heuristic utility of the model
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Reflexive metamodeling}: Models that incorporate self-criticism
\end{itemize}

\subsection{Final Reflection: The Finger and the Moon}

As taught by the \emph{Laṅkāvatāra Sūtra}:

\begin{quote}
\textit{"Words and teachings are like a finger pointing to the moon. The finger can indicate where the moon is, but the finger is not the moon. To see the moon, it is necessary to look beyond the finger."}
\end{quote}

This computational model is the finger. The direct experience of Bardo states is the moon. Not confusing one with the other is the wisdom that allows using the model effectively.
...
class QuantumAnalytics:
    ...
    def analyze_transitions(probabilities, threshold=0.1):
        """Unified analysis of transitions between states"""
...
class BardoQuantumSystem:
    """
    Complete quantum simulation system of Bardo Thodol
    WITH EXPLICIT DOCUMENTATION OF LIMITATIONS
    """
    ...
    def initialize_quantum_system(self):
        """Initializes base quantum system"""
        self.dimension = 3
        self.states = {
    ...
    def _create_operators(self):
        """Creates quantum operators for the system"""
    ...
    def run_complete_simulation(self):
        """Runs complete simulation with analysis"""
    ...
    def _classify_final_state(self, state):
        """Classifies final state according to probabilities"""
